...
setwd("D:/Science_Repository/science/2019_alternative_splicing/Ongoing_Virtual_Stiffness_on_TCGA_Glioma_Dataset/Data/Expression")
...
setwd("D:/Science_Repository/science/2019_alternative_splicing/Ongoing_Virtual_Stiffness_on_TCGA_Glioma_Dataset/Data/Annotation")
...
setwd("D:/Science_Repository/science/2019_alternative_splicing/Ongoing_Virtual_Stiffness_on_TCGA_Glioma_Dataset/Data/Virtual_Stiffness_gmt")
geneSets<-getGmt("Pathways.txt")
#Generating the zscore of ssGSEA
setwd("D:/Science_Repository/science/2019_alternative_splicing/Ongoing_Virtual_Stiffness_on_TCGA_Glioma_Dataset/Results")
...
#Generate the virtual stiffness
...
#write.table(virtual_stiffness,"virtual_stiffness.txt",quote=FALSE,sep='\t',row.names = TRUE,col.names = TRUE)
#virtual_stiffness <- read.delim("virtual_stiffness.txt",header=T,row.names = 1)

#Add virtual stiffness score to annotation_col to generate a new annotation datafram called virtual_stiffness
virtual_stiffness_anno <- merge(annotation_col,virtual_stiffness,by="Sample",all=FALSE)

#Add PIEZO1 and PIEZO2 expression to virtual_stiffness dataframe.
#PIEZO <- read.delim("D:/Science_Repository/science/2019_alternative_splicing/Ongoing_Virtual_Stiffness_on_TCGA_Glioma_Dataset/Data/Annotation/TCGA_B_PIEZO_expression.txt",header=T,row.names = 1)
#PIEZO <- scale(PIEZO)
#PIEZO <- as.data.frame(PIEZO)
#PIEZO$Sample <- row.names(PIEZO)
#virtual_stiffness_anno <- merge(PIEZO,virtual_stiffness_anno,by="Sample",all=FALSE)
#rownames(virtual_stiffness_anno) <- virtual_stiffness_anno$Sample
virtual_stiffness_anno$virtual_stiffness <- as.numeric(as.character(virtual_stiffness_anno$virtual_stiffness))
#save the virtual_stiffness dataframe which combined Virtual Stiffness, PIEZO1/PIEZO2 expression and TCGA glioma information together.
#write.table(virtual_stiffness,"Important_virtual_stiffness_PIEZO1_and_PIEZO2_in_TCGA_glioma.txt",quote = FALSE,sep='\t',row.names = TRUE,col.names = TRUE)

#Plot the heatmap, ssGSEA of virtual stiffness gene sets with selective annotations.
#virtual_stiffness_selected <- virtual_stiffness[,index]
...
colnames(selected_anno) <- c("Sample","virtual_stiffness","Grade","IDH.status","X1p.19q.codeletion",
                             "IDH.codel.subtype","MGMT.promoter.status","Chr.7.gain.Chr.10.loss",
                             "TERT.promoter.status","Supervised.DNA.Methylation.Cluster")
...
row_order <- c("GO_EXTRACELLULAR_MATRIX_ASSEMBLY","GO_CELL_MATRIX_ADHESION","GO_CELL_SUBSTRATE_ADHESION","GO_INTEGRIN_BINDING",
                                     ...
                                     "GO_COLLAGEN_FIBRIL_ORGANIZATION","GO_COLLAGEN_TRIMER", "GO_ACTIN_FILAMENT_ORGANIZATION","GO_ACTOMYOSIN_STRUCTURE_ORGANIZATION",
                                     "GO_ACTIN_CYTOSKELETON_REORGANIZATION")
ann_colors = list(
  virtual_stiffness = c("yellow","black","red"),
  Supervised.DNA.Methylation.Cluster = c("Classic-like"="#FF0000","Codel"="#00FF00","G-CIMP-high"="#0000FF","G-CIMP-low"="#FFFF00",
                                         "LGm6-GBM"="#FF8000","Mesenchymal-like"="#FF00FF","PA-like"="#00FFFF"),
